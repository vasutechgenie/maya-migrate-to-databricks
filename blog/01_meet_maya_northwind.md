...
`make demo` runs MAYA's **twelve-stage lifecycle** against a bundled example and writes
every artifact to `examples/northwind/out/`: a normalized dependency graph and
independently verified build order, a 100% traversability score, a whole-estate
test-catalog replication script filled with referential-integrity synthetic data, one
spec PDF per pipeline, the swarm-built and topologically certified `gates.json` (proven
first on a sampled dev catalog, then re-proven at full volume), the migrated BI layer
...
Everything MAYA does is a function of one normalized graph, and it runs as a **twelve-stage
lifecycle (0-11)**. Each stage runs its work, evaluates a gate, and refuses to advance
until that gate is green - so the estate is only ever built on a foundation that already
proved out. `make demo` runs the whole lifecycle with `maya run --stage all`:

0. **Readiness** - confirm the estate is ready: connectivity, identity, security, and
   governance inputs are collected and classified before any build begins.
...
2. **Replicate (dev)** - replicate every table and view into a Databricks test catalog,
   filled with referential-integrity-preserving data (synthetic 10k, or sampled from source).
3. **Specs** - render one branded build-spec PDF per pipeline.
4. **Build + certify (dev)** - a swarm of AI coding agents builds the *real* pipelines wave
   by wave and proves each on the sampled dev catalog, certifying in strict topological
   order (a pipeline certifies only after its predecessors do).
5. **BI convert (dev)** - convert the dashboards to Databricks SQL and dev-certify them on
   sampled gold.
6. **Full load + historical (prod)** - load full-volume, production-copied data (plus
   history) into the production catalog.
7. **Build + certify (prod)** - re-run the *same* pipeline code at full volume,
   point-in-time, then sustained - earning FINAL certification.
8. **BI parity + publish (prod)** - re-prove the dashboards at full volume and republish as
   Lakeview + Genie.
9. **Docs + publish** - generate full documentation for every pipeline, table, view, and
   dashboard, and publish it back to the repo.
10. **Identity + security + governance** - apply Unity Catalog groups and grants, masking,
    and secrets.
11. **Enablement + go-live** - training, runbooks, cutover/rollback, and day-2 ops.

Two ideas hold the twelve stages together. Build + certify is **two-phase**: the same code
...
dev (Stage 4) you build against a small **illusion of production** - every table, but only
a few thousand rows each - so you prove the logic is correct cheaply. Only then, at full
volume (Stages 6-7), do you re-prove the *same code* on production-copied data. And because
a pipeline that matches at cutover can still drift a week later, Stage 7 also runs both
systems in parallel and re-proves parity over time before final certification. We'll get
deep into that two-phase build+certify later in the series.

## Where we're headed

Over the next nine parts we'll follow Northwind through the lifecycle: how MAYA reads
...
catalog, and how the two-phase build+certify - proven on the sampled dev catalog, then
re-proven at full volume and sustained through soak - certifies each table in topological
order, up to the whole-system certification that declares the migration complete. We'll
finish with the two-phase BI/Genie migration, the live dashboard, generated docs, and
cutover - and how to point MAYA at your own estate.
