...
make demo
```
`make demo` runs seven phases against the bundled **Northwind** example and writes every
artifact to `examples/northwind/out/`.
...
5. **sample** - a small "illusion of production" for cheap logic proofs.
6. **validate** - parity across dev -> sit -> soak, with no partial credit.
7. **report** - a branded PDF.

## Why "MAYA"
*Maya* means "illusion". In dev you build against a small illusion of production (every table,
a few thousand rows) to prove logic cheaply, then prove it at full scale, then re-prove it over
time to catch drift. The three phases - Dev, SIT, Soak - are covered in Parts 7-9.
